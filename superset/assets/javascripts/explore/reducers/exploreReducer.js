...
export default function exploreReducer(state = {}, action) {
    // 去掉_dim后缀展示， 但不改变元数据（元数据保留_dim）
    const overrideDataSource = (data) => {
        if (data) {
            const newData = {...data};
            const overrideArrKey = ['all_cols', 'filterable_cols', 'gb_cols', 'order_by_choices'];
            overrideArrKey.map(i => {
                newData[i] = data[i].map(ii => {
                    return [ii[0], ii[1].replace('_dim', '')];
                });
            });
            if (data.columns) {
                newData.columns = data.columns.map(item => {
                    return {...item, verbose_name: item.column_name.replace('_dim', '')};
                });
            }
            return newData;
        }
    ...
    // 重写 datasource
    state = Object.assign({}, state, {datasource: overrideDataSource(state.datasource)});
    return state;
}
